...
func handleStatusRequest(c *gin.Context, config *Config) {
	...
	jobStatuses, err := getJobStatus(&data, config)
	if err != nil {
...
func getJobStatus(message *map[string]interface{}, config *Config) ([]JobStatus, error) {
	...
	jobList, err := clientset.BatchV1().Jobs(config.RecipeNamespace).List(context.TODO(), listOptions)
	if err != nil {
